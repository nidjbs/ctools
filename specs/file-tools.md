...
- 校验（纯函数 `shared/filePolicy.ts`）：`resolve` 后必须仍落在某个配置根内（`根/` 前缀）。
  - `checkInside(roots, target)` 成功 → 规范化绝对路径；否则 `null`。
  - `..` 穿越、非根内绝对路径 → 拒绝。
- 已知边界（Phase 1 记录）：仅做词法 containment，不做 symlink 逃逸防护；file_roots 为空时所有读都拒绝。
...
- 越界路径一律拒绝；symlink 逃逸不在词法校验内（记录为已知边界）。
