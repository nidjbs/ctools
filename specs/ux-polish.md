...
## §10 首启目录引导（file_roots 为空）

**问题**：`file_roots` 默认空 → 文件工具全部拒绝、`bash` 不可用，用户看不出该怎么办（"默认下没法执行任务"）。

**为什么不用 cwd 作默认**（已实测）：macOS 由 LaunchServices 启动的 `.app`，进程 cwd 是 **`/`** 或 app bundle 内部（实测微信=app bundle 内、另两个 GUI 应用=`/`），**从不是用户的项目目录**。若默认 cwd，打包后会静默把 `file_roots` 设成 `/` —— 等于把整个文件系统开放给 agent，直接击穿「file_roots 限定」不变量；且 dev（cwd=仓库）与打包（cwd=`/`）行为不一致。

**行为**：
- `file_roots` 为空时，Launcher 空输入态显示引导条：**「未配置可访问目录，文件功能不可用」** + 「选择目录」按钮。
- 点「选择目录」→ 主进程 `dialog.showOpenDialog({ properties: ['openDirectory','createDirectory'] })` 原生选目录 → 选中即写入 `config.fileRoots = [选中目录]`，引导条消失；取消则什么都不改。
- **不静默放宽任何权限**：目录由用户显式选择。
- 引导条可关闭（本次运行不再弹），与 §6 网关引导条同机制、可同时出现（网关在上）。
- 文件工具在 `file_roots` 为空时的报错改为可操作文案（指向设置），而非仅「路径不在 file_roots 内」。

**边界**：选目录对话框取消 → 返回 `null`，不改配置；写入失败 → 静默（下次唤起仍会引导）。
