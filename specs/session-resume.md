...
Chat 窗口关闭（销毁）时，主进程必须**立刻释放挂起状态**，而不是让它们等到超时：

- 未决的**工具批准**（`pendingConfirms`）→ 全部按**拒绝**结算。
- 未决的**提问**（`pendingAsks`）→ 全部按**未回答**结算。
- 当前轮**中止**（`chat.cancel()`）——窗口已不存在，继续跑既无人看见也无人能批准。

**为什么必须这样**：未决请求的超时是 120s（批准）/ 300s（提问），期间 `ChatManager.running` 恒为 `true`；
而 `attach`/`newSession` 都有 running 守卫且**先于**开窗执行 → 它们会抛错、窗口不开，
表现为「点最近会话毫无反应」。关窗即释放是唯一能消除这段悬空窗口的做法。

## 失败语义

- id 不存在 / 文件不可读 → 抛错；UI **必须显式呈现**该错误（曾经是静默吞掉，表现为「点了没反应」）。
- running 中 attach/new → 抛错；UI 提示等待或停止。
  - 注：正常关窗路径下不会撞上此守卫——关窗已释放 running（见 C5）。
