...
- **长期记忆**：让 agent「记住…」你的偏好与事实，跨会话生效——索引常驻、正文按需召回、可设为常驻；内容随时可在设置里审阅/删除
...
> 状态：**0.5.0 —— 首个可分发版本**。自带模型网关，装完即可用；有单测 + 运行时 e2e + 黄金集 + UI e2e 四层回归与 CI 门禁。
> **使用说明见 [docs/guide.md](docs/guide.md)**；版本变更见 [CHANGELOG.md](CHANGELOG.md)；后续规划见 [docs/roadmap.md](docs/roadmap.md)。
> 当前仅支持 **macOS**，且**产物未签名**（首次打开需右键「打开」，详见指南）。
...
首次使用：Launcher 空态会出现两条引导条 —— **「未配置可访问目录」→ 选择目录**（决定文件工具与 `bash` 能作用于哪里），以及（若网关未连）**「模型网关未连接」→ 打开设置**。也可以随时在**设置**里改 gateway 地址、别名与目录。

> **为什么目录要你选**：macOS 上 `.app` 的进程工作目录是 `/`（不是你的项目目录），若拿它当默认等于把整个文件系统开放给 agent。所以 cTools **不猜**——由你显式指定，文件工具与 `bash` 一律以它（`file_roots`）为界；未配置时文件类能力不可用，并在 Launcher 给出引导。
...
- **模型**：默认模型别名 + **各场景模型**（如翻译可单独用一个模型，留空即用默认）+ 剪贴板召回模型（建议指向本地模型，隐私）。
- **网关配置**：直接编辑网关自己的 **上游 providers** 与 **模型别名 aliases**（`~/gw.yaml`）——加一个上游、加一个模型别名不必再手编 YAML。保存时**自动备份**原文件、只改动这两块、其余键原样保留；`api_key_env` 只填**环境变量名**，cTools 不存储也不回显真实密钥。改完「保存并热更」（或「保存并重启」）。
- **连接（高级，默认折叠）**：`gatewayUrl` / `adminUrl` / `adminToken`。默认值（`127.0.0.1:8080/8081`）就是正确值，**通常无需修改**——填错会让命令与对话全部不可用。
- **偏好**：全局热键、文件根目录 `file_roots`、写确认模式、`bash` 联网开关（默认关）、每天日期注入开关。
- **记忆**：列出 agent 记住的内容，可删除、可设为「常驻」（常驻会把正文每轮发给模型，默认关）。
...
| `remember` | 记住 · memorize | 让 agent 记住一条事实/偏好，跨会话生效 |
...
- **记忆可控**：索引（标题+一句话）常驻，但**正文只在 agent 主动检索时才进入模型**。勾选「常驻」会把正文每轮都发给当前模型（可能为远端）——设置页有明确提示，请只对不敏感内容开启。记忆随时可审、可删（删除是追加墓碑，原文仍可审计）。
...
| `memory.jsonl` | 长期记忆（追加式，删除也是追加一条 tombstone；原文永久保留可审计） |
| `MEMORY.md` | 记忆索引页（由 `memory.jsonl` 自动生成；手改会被下次写入覆盖） |
| `spill/` | 超长工具结果的外置全文（agent 可用 `file_read` 回取；**只读、不可写**） |
