...
- **cTools**：你的个人 AI 助手，一个悬浮命令面板，把简单、重复的工作收敛到热键输入框。
...
3. **LLM 后端 = 本地模型网关**（OpenAI 兼容 HTTP），唯一模型出口。
4. **安全（远端模型不可信）**：
...
- **能力内建**：一切能力都是 cTools 内的 TS `Command`（commands/ 注册），不依赖外部 CLI 移植；本地模型网关只是 LLM 后端。
- **依赖**：少而精；新增依赖需说明理由。
